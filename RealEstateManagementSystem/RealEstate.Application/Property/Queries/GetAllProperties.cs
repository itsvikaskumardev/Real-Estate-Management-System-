        ...
        : IRequestHandler<GetAllPropertiesQuery, PaginatedList<PropertyDto>>
    {
            ...
            CancellationToken ct)
        {
            try
            {
                ...
                var allProperties = await projectedQuery.ToListAsync(ct);
                ...
                if (!string.IsNullOrWhiteSpace(request.Amenities))
                {
                    ...
                    {
                        allProperties = allProperties.Where(p => p.Amenities != null && p.Amenities.Contains(amenity)).ToList();
                    }
                }

                var count = allProperties.Count;
                var pageNum = (request.PageNumber ?? 1) > 0 ? (request.PageNumber ?? 1) : 1;
                var pageSz = (request.PageSize ?? 12) > 0 ? (request.PageSize ?? 12) : 12;
                var pagedItems = allProperties.Skip((pageNum - 1) * pageSz).Take(pageSz).ToList();

                return new PaginatedList<PropertyDto>(pagedItems, count, pageNum, pageSz);
            }
            catch (Exception ex)
            {
                System.IO.File.WriteAllText(@"c:\Users\VikasKumar1\OneDrive - SupplyIT\Documents\Vikas\REALSTATE\error.log", ex.ToString());
                throw;
            }
        }
    }
